...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	public void setChangeTextColor(boolean isChangeTextColor) {
			...
			if (pager.getAdapter() instanceof IconTabProvider) {
				addIconTab(i,
						((IconTabProvider) pager.getAdapter())
								.getPageIconResId(i));
			} else if (pager.getAdapter() instanceof CustomViewProvider) {
				addCustomViewTab(i,
						((CustomViewProvider) pager.getAdapter())
								.getPageViewResId(i));
			} else {
	...
	private void addCustomViewTab(final int position, int resId) {
	 ...
	 * @param position
	 */
	private void updateTextStyles(int position) {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (v instanceof TextView && i == position) {
				...
				TextView tab = (TextView) v;
				tab.setTextColor(underlineColor);
			} else if (v instanceof TextView && i != position) {
				TextView tab = (TextView) v;
				tab.setTextColor(tabTextColor);
			}
		}
	 ...
	 * @param id 图片ID
	 */
	public void updateTextDrawNotice(int position, int id) {
	 ...
	 * @param id 
	 */
	public void updateImgTitleDrawNotice(int position, int count) {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (v instanceof View && i == position) {

				TextView tab = (TextView) v.findViewById(((CustomViewProvider) pager.getAdapter()).getPageTxtResID());
				if (count != 0) {
	...
	public void updateTabViewStyle(int position) {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (v instanceof View && i == position) {
				...
				View tab = v;
				ImageView head = (ImageView) tab
						.findViewById(((CustomViewProvider) pager.getAdapter())
								.getPageImageLayoutResID());
				if (head!=null) {
					head.setImageResource(((CustomViewProvider) pager.getAdapter())
							.getCurPageImageResID(i));
				}
	...
	private class PageListener implements OnPageChangeListener {
		...
		public void onPageSelected(int position) {
			...
			updateTextStyles(position);
			updateTabViewStyle(position);
		}
